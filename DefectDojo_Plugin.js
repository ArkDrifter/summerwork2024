...
(function () {
  ...
  const config = {
    openproject: {
      url: "http://192.101.101.68:8085",
      apiKey:
        "df5f67456dbcd3de70c7b541c9b7c0ffc09778cd8e3822c5124b9e2e025079f3",
    ...
    defectdojo: {
      ...
      apiPath: "/api/v2/",
      token: "Token 548afd6fab3bea9794a41b31da0e9404f733e222",
    },
  };
  ...
  function parseOpenProjectData(data) {
    return data._embedded.elements
      .map((task) => {
        // Более гибкое извлечение ID уязвимости из описания задачи
        // Поддержка различных форматов URL DefectDojo
        let findingUrl = "N/A";
        let findingId = null;

        if (task.description && task.description.raw) {
          // Ищем любые URL, содержащие /finding/ с числовым ID
          ...
          // Альтернативный формат (в случае, если URL отличается)
          if (!findingId) {
            ...
            if (idMatches && idMatches.length > 1) {
              ...
              // Если URL не найден, но ID найден, формируем URL
      ...
      .filter((task) => task !== null); // Отфильтровываем null записи, если такие появятся
  }
  ...
  function createTooltips(data) {
    fetchOpenProjectProjects((projects) => {
      ...
      checkboxes.forEach((checkbox) => {
        ...
        if (tasks.length > 0) {
          console.log("Tasks for finding", checkbox.id, ":", tasks);
          ...
          tasks.forEach((task, index) => {
            // Получаем информацию о проекте прямо из задачи
            let projectId = "";
            ...
            // Добавляем визуальный бейдж для статуса
            ...
            // Добавляем строку с исполнителем
          ...
          const headerDiv = document.createElement("div");
          headerDiv.className = "custom-tooltip-header";
          headerDiv.textContent = "Информация о задачах";
          tooltip.appendChild(headerDiv);

          const contentDiv = document.createElement("div");
          contentDiv.className = "custom-tooltip-content";
          contentDiv.innerHTML = "Нет связанных задач в OpenProject";
          tooltip.appendChild(contentDiv);
  ...
  function createOpenProjectTaskIfNotExists(finding, openprojectTasks) {
    ...
    selectedProjectHrefs.forEach((projectHref) => {
      ...
      if (taskExists) {
        showNotification(
          `Task for finding ID ${id} already exists in selected project. Skipping creation.`,
          "warning"
        );
        return;
      }
